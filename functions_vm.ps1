...
function VSA-VM {
    ...
    if ($vm) {
        ...
        Write-Host "Sockets: " -nonewline
        if ($($vm.ExtensionData.Config.Hardware.NumCPU / $vm.ExtensionData.Config.Hardware.NumCoresPerSocket) -lt 2) {
            
            Write-Host "$($vm.ExtensionData.Config.Hardware.NumCPU / $vm.ExtensionData.Config.Hardware.NumCoresPerSocket)" -f yellow
        }
        else {
            Write-Host "$($vm.ExtensionData.Config.Hardware.NumCPU / $vm.ExtensionData.Config.Hardware.NumCoresPerSocket)" -f green
        }
        Write-Host "RAM: " -nonewline
        if ($vm.MemoryGB -lt 8) {
            Write-Host "$($vm.MemoryGB)" -f red
        }
        elseif ($vm.MemoryGB -ge 8 -and $vm.MemoryGB -lt 16 ) {
            Write-Host "$($vm.MemoryGB)" -f yellow
        }
        else {
            Write-Host "$($vm.MemoryGB)" -f green
        }
